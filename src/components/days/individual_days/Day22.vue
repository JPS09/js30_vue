...
<script>
// Select all to be highlighted elements
const triggers = document.querySelectorAll("a");

//Create a span which will highlight above elements
const highlight = document.createElement("span");
highlight.classList.add("highlight");
document.body.append(highlight);

// Function that get the current hovered element and move the span accordingly
function hightlightLink() {
  // Get the current element informations
  const linkCoordinates = this.getBoundingClientRect();

  // Adjust for user scrolling to avoid offset
  const coords = {
  ...
  // Using Css to move the span to the current element to highlight it.
  highlight.style.width = `${coords.width}px`;
  highlight.style.height = `${coords.height}px`;
  highlight.style.transform = `translate(${coords.left}px,${coords.top}px)`;
}

// Run the above function when the mouse enter the bouding box of the element.
