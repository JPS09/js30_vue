...
<script>
// TODO Improve the animations of this exercise
...
function handleEnter() {
  ...
  setTimeout(() => {
    // Check if the dropdown has a trigger enter class before applying the active one to avoid showing the content without a background
    this.classList.contains("trigger-enter") &&
      this.classList.add("trigger-enter-active");
  }, 150);
  // Adds a class to make the background appear
  background.classList.add("open");
  // Select the hovered dropdown
  const dropdown = this.querySelector(".dropdown");
  // Get the relative position of this element
  const dropdownCoords = dropdown.getBoundingClientRect();
  // Get the parent nav position
  const navCoords = nav.getBoundingClientRect();

  const coords = {
    width: dropdownCoords.width,
    height: dropdownCoords.height,
    // Substract the nav coords to avoid offset.
    top: dropdownCoords.top - navCoords.top,
    left: dropdownCoords.left - navCoords.left,
  };

  // Applies the coords to the background to make it follow the dropdown
...
function handleLeave() {
  // Opacity 1 to 0 for transition
  this.classList.remove("trigger-enter-active");
  setTimeout(() => {
    // display block to non
    this.classList.remove("trigger-enter");
  }, 50);
  // Remove a class to make the background disappear
